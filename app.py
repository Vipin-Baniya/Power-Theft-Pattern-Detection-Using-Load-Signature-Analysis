...
def ensure_models(ROOT, MODEL_DIR):
    ...
    This runs training synchronously and returns (success, log)."""
    ...
    try:
        ...
        if proc.returncode == 0:
            ok = all(Path(MODEL_DIR, f).exists() for f in needed)
            if ok:
                return True, out
            else:
                return False, "Training finished but artifacts still missing.\n" + out
        else:
            return False, "Training script failed (non-zero exit).\n" + out
    except Exception as e:
        return False, f"Exception while running training: {e}"

# At app start, ensure models exist (only when running as Streamlit app)
try:
    # ROOT and MODEL_DIR are defined below in the original file; define placeholders to avoid NameError
...
if st.sidebar.button("Retrain (fast demo)"):
    with st.spinner("Running fast retrain..."):
        ok, log = ensure_models(ROOT, MODEL_DIR)  # this will run fast train because ensure_models uses --fast
        if ok:
...
if st.sidebar.button("Full retrain (slow)"):
    # run full training explicitly
    import subprocess, sys
    with st.spinner("Running full retrain (this may take longer)..."):
        cmd = [sys.executable, "src/train_model.py", "--full"]
        proc = subprocess.run(cmd, cwd=ROOT, capture_output=True, text=True, timeout=1800)
        out = proc.stdout + "\n" + proc.stderr
        if proc.returncode == 0:
            st.success("Full retrain finished; reloading app.")
            st.experimental_rerun()
        else:
            st.error("Full retrain failed. See log below.")
            st.code(out[:10000])
...
# Ensure model artifacts are present; if missing, run training automatically.
success, train_log = ensure_models(ROOT, MODEL_DIR)
if not success:
    st.warning("Model artifacts were missing; attempted to train but failed. See log below.")
    st.code(train_log[:10000])
else:
    # show brief note if training ran
    if "Saved model and artifacts to" in train_log or "Accuracy:" in train_log:
